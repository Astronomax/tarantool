...
local g1 = t.group('deprecate_replication_synchro_timeout')
--
...
g1.test_option_behaves_as_before_if_old = function(cg)
    cg.server:restart({
        alias = 'old_behavior',
        env = get_env('old'),
    })
    cg.server:exec(function()
        ...
        box.cfg{ replication_synchro_timeout = 239, }
        t.assert_equals(box.cfg.replication_synchro_timeout, 239)
    end)
end

g1.test_option_deprecated_if_new = function(cg)
    cg.server:restart({
        alias = 'new_behavior',
        env = get_env('new'),
    })
    cg.server:exec(function()
        ...
        t.assert_error_msg_content_equals(
            "Option 'replication_synchro_timeout' is deprecated",
...
g1.test_switch_option_to_new = function(cg)
    cg.server:exec(function()
